...
5. Local Google Agents CLI skills (globally installed `google-agents-cli-*`) are structural and workflow examples, not Agent Factory authority. **Keep this rule stated explicitly.** Both skill families are visible in the same turn: the globally installed Google set and this repository's `af-*` set, and both claim to be the always-active development lifecycle. Measured once: a strong model read both sets, then cited *this line* as its reason for following the `af-*` lifecycle over `agents-cli scaffold create`. No runtime-level ranking of repository skills over global ones was observed, and whether one exists was not tested — so do not assume the ordering survives if this entry is deleted as redundant.
