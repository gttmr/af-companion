...
Discover Phase A makes no repository or Work Item write. It ends with a Discovery Decision Plan and runs `companion prepare-materialization`. When actual discovery/decision revisions already exist, the Bridge prepares a re-entrant Handoff bound to those revisions, the source Work Item ETag, exact source session/latest turn, Plan hash, target, expiry, and marker checksum. When the Work Item is still the exact strict default ledger, it prepares one Bootstrap Grant instead; do not invent revisions.

A fresh materialization session must present an exact marker on its first prompt. Claim only one unexpired authority after its exact scope/hash/target/expiry and source match, the new session differs from the Plan session, and it was not already claimed. Re-entrant Handoffs also require unchanged source ETag/revisions and the latest source turn; Bootstrap Grants require the unchanged pristine ETag and latest source turn, and may survive Bridge restart while the preserved source record remains non-revoked. Duplicate, expired, superseded, ambiguous, stale, wrong-work-item, wrong-cwd, or Plan-hash-mismatched claims stop materialization.

The Bridge's observed authority and the Work Item's durable `session_handoffs` are related evidence, not interchangeable shapes. A re-entrant Handoff claim authorizes Phase B to append one claimed record with its source revision tuple and complete identity/claim provenance; an exact pre-existing pending record is updated instead. A Bootstrap Grant claim authorizes Phase B to write the real revisions and exactly one claimed Handoff record with the Grant identity/source/hash/times/claim provenance; the Bridge then auto-finalizes only on an exact match. Re-read both plus the approved Plan before writing. Bridge health alone does not prove first-prompt delivery or claim.
