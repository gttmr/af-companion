...
export function AppWorkspaceBar({ apps, busy, notice, onActivate, onCreate }: {
  ...
  return <div className="app-workspace-bar">
    <div className="app-identity"><span>Active App</span><select aria-label="Active App" value={apps.active_application_id ?? ""} disabled={busy || apps.apps.length === 0} onChange={(event) => void onActivate(event.target.value)}><option value="" disabled>App 선택</option>{apps.apps.map((app) => <option key={app.application_id} value={app.application_id}>{app.display_name} · {app.application_id}</option>)}</select><button type="button" className="text-button" onClick={() => setOpen((value) => !value)}>새 App</button></div>
    ...
    {open ? <form className="new-app-form" onSubmit={(event) => void submit(event)}>
