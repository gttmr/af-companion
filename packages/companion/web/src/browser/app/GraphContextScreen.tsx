...
import { DevelopmentTaskPanel } from "../development/DevelopmentTaskPanel.js";
...
export function GraphContextScreen({ api: providedApi }: { api?: CompanionApi }) {
  ...
  return <main className="workbench-shell">{shellHeader}<ContextPublicationStrip workspace={state.workspace} pending={publishing} /><nav className="canvas-toolbar" aria-label="Graph layout controls"><button type="button" onClick={autoLayout}>자동 Layout</button><button type="button" onClick={autoLayout}>Layout 초기화</button><button type="button" onClick={fitSelection} disabled={state.selection?.kind !== "node"}>선택에 맞춤</button><span>Graph identity와 workflow_ref는 읽기 전용입니다.</span></nav><AssetLibrary api={api} graph={state.draftGraph} appAssets={appAssets} onAssetsChange={setAppAssets} onStage={stage} /><CreationPanel graph={state.draftGraph} onStage={stage} /><div className="workspace-grid"><GraphCanvas graph={state.draftGraph} presentation={presentation} selection={state.selection} changed={highlights} onSelectionChange={(selection) => void select(selection)} onPositionChange={(nodeId, x, y) => setPresentation((current) => current ? { ...current, positions: { ...current.positions, [nodeId]: { x, y, pinned: true } } } : current)} /><ElementInspector graph={state.draftGraph} selection={state.selection} assetBindings={appAssets.bindings} errors={state.fieldErrors} onStage={stage} onDelete={remove} developmentPanel={state.selection ? <DevelopmentTaskPanel key={apps.active_application_id} api={api} applicationId={apps.active_application_id} graphRevision={state.workspace.graph_revision} selection={state.selection} /> : null} /></div><footer className="workspace-footer"><span>{state.workspace.source_health.status === "valid" ? "Graph validation · 통과" : "Graph write · 차단"}</span><span className="notice-copy" role="status" aria-live="polite">{state.notice}</span>{state.phase === "conflict" ? <button type="button" className="text-action" onClick={() => void loadWorkspace("최신 Graph를 다시 읽었습니다.")}>최신 Graph 불러오기</button> : null}</footer></main>;
}
...
function ConnectionHelp() { return <details className="connection-details"><summary className="button-secondary">Codex 사용 안내</summary><div className="connection-popover"><strong>Active App의 외부 Codex Graph Tool</strong><p>먼저 현재 App을 VS Code에서 여세요. project-local MCP 설정은 그 App root에만 적용됩니다.</p><p>Workspace Trust와 별도로 Codex가 exact App root를 trusted project로 인식해야 `companion_graph`가 로드됩니다.</p><p>Codex extension에서 새 chat을 시작하고 `companion_get_graph_workspace`를 호출합니다. Graph 변경은 get → apply 순서이며 write Tool은 승인을 요청합니다.</p><p>VS Code 실행 receipt는 Codex thread 연결이나 lifecycle authority를 의미하지 않습니다.</p></div></details>; }
