...
- Browser code uses `/api/companion/v2` and SSE for Graph collaboration. The
  separate Web composition action opens the server-derived demo project in
  VS Code; browser input cannot select a filesystem path.
- Node startup composes `GraphControlWorkspace` and its loopback HTTP server; all
  persistence and concurrency behavior remains owned by
  `graph-control-server`.
...
| `POST` | `/api/companion/editor/launch-vscode` | open the canonical demo project in a new VS Code window |
