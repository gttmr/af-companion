...
- `context/`에는 strict Target Contract v2의 `analysis-result.json`, `normalized-requirement.json`, `asset-candidates.json`, `graph-ir.json`, `scaffold-plan.json`, `af-work-item.json`, `boundary-design.md`가 서로 일치하는 상태로 존재한다.
...
- strict current generator가 reviewed env-backed external HTTP Tool을 지원하지 않으므로 `can_generate_source=false`, Scaffold `blocked`로 fail closed 한다.
