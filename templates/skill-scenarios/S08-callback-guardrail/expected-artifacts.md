...
- strict Target Contract v2 analysis, derived artifact, `af-work-item.json`, boundary design, scaffold plan을 predecessor evidence로 확인한다.
- `adk_callback` 계약이 설계 검토됐다는 사실과 generic runnable generator가 callback control을 지원하지 않는다는 사실을 분리한다.
- `scaffold-plan.json`의 `can_generate_source: false`, callback lowerer blocker, Scaffold `blocked`를 그대로 보고한다.
