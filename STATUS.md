...
- Home now owns the Web-first Plan start UI: one application name or existing Work Item, one primary VS Code action, explicit new-path confirmation, and Trust/MCP guidance tied to fresh session/tool evidence. Connections has no manual enrollment form, fresh-session command, Capsule copy, or Capsule DOM rendering; existing-session Attach, Cancel, Revoke, and diagnostics remain. Discover launches Materialization from the latest exact canonical Handoff or strict pristine-ledger Bootstrap Grant. The Grant binds ETag/source turn/Plan hash/expiry/one-time fresh claim and auto-finalizes only after Phase B writes the actual matching claimed Handoff record. The complete first journey remains live-unverified after the P7 blocker fix.
