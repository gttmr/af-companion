...
- `packages/companion` is now the primary Companion development and acceptance
...
- The guarded terminal-session launch resolves that binding, generates an ignored mode-`0600` app-first/factory-second `.code-workspace`, and invokes `code --new-window` only after confirming the Bridge/editor path. Workspace Trust lets its `folderOpen` Task run `af companion vscode-start`; the CLI, not the browser, issues the `af_vscode_launch` ticket and launches factory-cwd Codex with the app sandbox writable root plus per-session `on-request` approval. Sandbox network remains off; only an exact user-approved command may cross that boundary to the loopback Bridge. Browser launch carries no Capsule and starts no turn. Approval is transport capability, while the internal `plan` role and current prompt receipt remain separate lifecycle evidence; neither forces Codex Plan mode.
- Home now owns the Web-first terminal start UI: one application name or existing Work Item, one primary VS Code action, explicit new-path confirmation, and Trust/MCP guidance tied to fresh session/tool evidence. Connections has no manual enrollment form, fresh-session command, Capsule copy, or Capsule DOM rendering; existing-session Attach, Cancel, Revoke, and diagnostics remain. Discover may separately launch Materialization from the latest exact re-entrant Handoff or strict pristine-ledger Bootstrap Grant, but that convenience path is not terminal connection proof.
...
- Every accepted leased top-level prompt receives a non-secret current-prompt participation receipt through Hook context, making exact session/turn/scope/lease provenance visible in that turn. Grant/Handoff authority uses the enrolled lifecycle `role: plan`, not Codex `permission_mode`; collaboration mode remains diagnostic metadata and cannot grant lifecycle authority.
- Fresh-session transfer is explicit. `companion prepare-materialization` creates a Bridge-local re-entrant Handoff for a non-pristine Work Item with current discovery/decision revisions, binding exact source ETag, ID, marker, revisions, verified Plan body, latest Plan turn, and consume-once Capsule without a Phase A ledger write; its Plan remains encrypted and unclaimed authority fails on drift or Bridge restart. A strict pristine Work Item instead receives a Bootstrap Grant using its default-ledger ETag, source Plan session/latest turn, Plan hash, expiry, and one-time fresh claim. The Grant survives restart, temporarily keeps the Plan plaintext only in ignored mode-`0600` local state, omits it publicly, and auto-finalizes against one exact claimed Work Item Handoff record. `/connections` existing-session Attach remains Handoff-only and never preselects a target.
...
- `/api/codex-companion` — Plan or exact Handoff/Bootstrap Grant multi-root VS Code descriptor launch, enrollment, Companion sessions, Handoff attach/cancel, revocation, and scoped next-prompt deliveries;
...
- [Smart CEP Companion/ADK continuation record](docs/workbench/follow-ups/19-smart-cep-companion-adk-continuation.md)
