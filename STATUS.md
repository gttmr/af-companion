...
- Fresh-session transfer is explicit: use Companion Continue when built-in Capsule carriage is not proven. Claims bind the exact canonical Work Item Handoff ID, marker, revisions, and verified Plan body, and are consume-once. The bounded body is encrypted in local state and erased on terminal authority changes. `/connections` can also durably attach a pending Handoff to one explicitly selected same-scope existing session for its next leased prompt, without returning a raw Capsule or Plan body; no target is preselected.
- Decision input is selected from tools actually exposed in the current turn. Structured and conversational paths normalize to the same semantics, while strict Decision and Asset Decision records preserve decision/recommendation revisions, selection source, bounded answer summary, input mode, and exact session/turn; a recommendation is never consent.
