...
- Empty Work Item bootstrap, Graph, Registry, enrollment, Handoff/Grant workspace launch, session revocation, delivery, state reset, and editor mutations additionally require same-origin requests.
...
- A Materialization Bootstrap Grant is available only for the exact strict default ledger. It binds the pristine ETag, exact source Plan session/latest turn, canonical Plan hash, target, expiry, and a rotated consume-once claim for one distinct fresh session. The Plan is temporary plaintext in ignored mode-`0600` Bridge state, omitted from public/browser/workspace bytes, erased on claim/failure/expiry/supersession, and deliberately survives Bridge/host restart while the source record remains exact and non-revoked. This is a local single-user integrity boundary, not protection from a hostile same-user process. Actual revisions plus one exact claimed canonical Handoff record are required for automatic finalization.
