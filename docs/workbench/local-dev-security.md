...
- Handoff creation accepts at most 64 KiB of canonical Plan text, rejects Capsule markers, recomputes its hash, and binds the exact canonical Work Item Handoff ID and marker. The body is AES-256-GCM ciphertext under a per-process in-memory key, never appears in public snapshots/receipts, and is erased on claim or any terminal authority transition. Restart cannot decrypt stale bodies and fails pending authority closed.
