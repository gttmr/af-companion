...
VS Code extension은 가능한 client host 중 하나일 뿐 필수 조건이 아니다. 현재 primary
surface에는 Browser workspace와 Node composition entrypoint가 있으므로 첫 번째 구현 가설은
`packages/companion/web` backend가 `codex app-server` child process를 `stdio`로 소유하는
구조다. 이 가설이 성립하면 Browser UI를 유지하면서 별도 terminal transport 없이 실행을
직접 소유할 수 있다. 최종 host는 이 가설의 검증 결과와 실제 제품 요구로 결정한다.
...
| --- | --- | --- | --- |
| primary Companion Web backend | active App root와 server-side process composition을 이미 가짐 | Browser에 event/approval을 전달할 얇은 channel 필요 | 첫 feasibility 대상 |
...
- `packages/companion/app-server-client`: 현재 protocol handshake, thread/turn, event와
  failure validation 경계
- `packages/companion/web`: active App root, Browser projection, VS Code convenience launch와
  Node process composition 경계
- `packages/companion/graph-control-server`: App/Graph persistence와 execution plane 사이에
  섞이면 안 되는 canonical writer 경계
- legacy `packages/web`의 launcher, Bridge, Hook, `scripts/af.mjs`: 간접 transport가
  해결하던 scope, recovery, review 무결성의 reference
...
첫 작업은 전체 Companion을 교체하는 프로젝트가 아니라 **primary Companion backend가 App Server
...
legacy lifecycle에서 관찰된 launch, state, recovery failure는 direct-ownership slice가
대체해야 할 acceptance evidence로만 사용한다. 기존 launcher와 Bridge를 영구적으로
정교화하는 작업을 이 primary 변경에 섞지 않는다.

Smart CEP/ADK 구현은 별도 work order다. Companion architecture가 최소 acceptance를 통과한
뒤 재개할 때 canonical Work Item과 artifact revision을 다시 읽으며, 과거 receipt, lease,
Handoff 상태를 새 실행의 권한으로 재사용하지 않는다.

## 작업을 시작할 때의 판단 요약

다음 작업자는 “VS Code extension을 구현한다”는 전제를 받지 않는다. 대신 현재 source와
공식 App Server schema를 읽고, primary Companion backend가 `stdio` child를 직접 소유하는 가장 작은
