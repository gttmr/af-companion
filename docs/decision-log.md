...
- **결정**: Hook 관찰만으로 Codex Session을 Agent Factory 참여자로 등록하지 않는다. Workspace Eligibility, Session Participation, Work Attachment를 독립 축으로 두고, `companion_active` Session은 one-time Enrollment Ticket과 short-lived Session Lease를 통해 exact canonical cwd·workspace·application·Work Item·role·session·Bridge instance에 결합한다. `unmanaged`는 durable Session 상태가 아니며 Hook process가 실행되더라도 AF endpoint 탐색·network·state/activity write 없이 종료한다. Context Delivery는 active participation, 유효 Lease, 동일 scope, 허용 role, canonical Work Item과 current bundle revision을 모두 확인한다. Global default target과 first-active/sole-pending 자동 선택은 제거한다. Plan body hash와 signed Handoff Capsule을 분리하고 built-in fresh-context 운반이 검증되기 전에는 explicit Companion Continue를 기본으로 사용한다. 별도 사용자 동작으로만 pending Handoff를 current same-scope materialization Lease에 영속 Attach할 수 있으며, 이 경로는 raw Capsule을 반환하지 않고 exact target의 다음 leased prompt에서 한 번 claim한다. `request_user_input`은 현재 Turn에 실제로 제공될 때만 Structured Adapter로 사용하고, 없으면 한 Turn 한 질문·`waiting_for_input`·Turn 종료를 지키는 Conversational Adapter로 같은 Decision ID/revision/option 의미를 기록한다. Bridge interaction state는 breaking v2이며 v1 Session을 조용히 승격하거나 migration하지 않는다. (대체: 2026-07-24 Plan-driven Discovery 항목의 Hook-observed Session 등록과 explicit marker/attach-only 경계, 2026-07-22 Project Hook Session Registry 항목의 observed-session recovery와 global default target)
...
- **영향**: `packages/web` Bridge state/API/types, Hook adapter와 root CLI launcher, Companion plugin, Connections/live rail, 다섯 Work Skill과 shared references, Operating Model·Companion·Security·Design System·Handbook·README/STATUS를 함께 갱신한다. 일반 Codex Session에는 AF side effect가 없지만, project/plugin Hook process 자체가 실행되지 않는 strict mode는 dedicated `CODEX_HOME`의 실제 CLI/IDE process trace 전까지 지원으로 선언하지 않는다. Raw prompt/transcript/tool input/output은 저장하지 않고 activation Capsule 외 prompt bytes를 Bridge로 전달하지 않는다.
