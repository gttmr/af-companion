...
| Workspace eligibility | canonical repository/Bridge startup | enrollment API and Hook workspace gate | current factory or explicitly registered application; never inferred as participation | `codexBridgeStore.ts`, `af-codex-hook.mjs` |
...
| Handoff Capsule | current Plan session/turn | distinct fresh materialization session | digest and bounded metadata in Bridge v2; claim token consume-once, Plan body hash excludes Capsule | `codexBridgeStore.ts`, `af-codex-hook-protocol.mjs` |
